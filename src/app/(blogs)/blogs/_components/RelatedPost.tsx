...
const RelatedPost = ({ posts }: RelatedPostProps) => {
  return (
    <div className="flex items-stretch gap-4 px-4 sm:px-0 overflow-x-auto scrollbar-thin scrollbar-thumb-primary-200 scrollbar-track-transparent scrollbar-thumb-rounded-xl lg:flex-col">
      {posts.map((post) => (
        <div
          className={`border border-secondary-200 bg-secondary-50 rounded-xl overflow-hidden flex flex-col lg:flex-row-reverse lg:items-center lg:gap-x-4 lg:rounded-none lg:border-none lg:bg-transparent lg:w-full ${
            posts.length > 1 ? "min-w-[90%]" : "min-w-full"
          }`}
          key={post._id}
        >
          <CoverImage
