...
const Modal = ({ children, title, description, open, onClose, className }: ModalProps) => {
      ...
      >
        <div className="pb-4 border-b border-secondary-200 mb-6">
          <div className="flex items-center justify-between text-secondary-700">
            ...
            <button onClick={onClose} type="button" className="p-2 -m-2" aria-label="Close">
              <XMarkIcon className="w-6 h-6" />
            </button>
          </div>
          {description && (
            <p className="text-primary-400 text-sm lg:text-base mt-2">{description}</p>
          )}
        </div>
        <div>{children}</div>
      </div>
