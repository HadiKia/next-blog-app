  ...
  return (
    <>
      ...
      <aside
        ...
        aria-hidden={!open}
      >
        <div className="flex items-center justify-between py-2 mb-6 border-b border-secondary-200">
          <div className="relative fill-primary-800 w-14 ">
            <svg
        ...
        <div className="flex flex-col gap-y-3 pb-8">
          {navLinks.map(({ id, path, children, Icon }) => (
            <NavLink
              key={id}
              path={path}
              icon={Icon}
        ...
        <div
          className="border-t border-secondary-200 pt-4 pb-6 mt-auto"
          onClick={() => setOpen(false)}
        >
          <UserSection />
        </div>
